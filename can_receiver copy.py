...
class CHAIReceiver:
    ...
    def scan_devices(self):
        ...
        print(f"Итого доступных каналов: {available_channels}")
    ...
    def connect(self, channel=0, baud_rate=500000):
        ...
        try:
            ...
            if self.is_connected and self.channel is not None:
                self.disconnect()
                time.sleep(0.5)
            ...
            if channel not in devices:
                raise Exception(f"Канал {channel} не найден")
            ...
            print("⚙️ Конфигурирование канала...")
            btr0, btr1 = self._get_btr_settings(baud_rate)
            self.chai.CiSetBaud(channel, btr0, btr1)
            self.chai.CiSetFilter(channel, 0, 0)
            print(f"   ✅ Скорость {baud_rate} bps установлена")
...
class DemoReceiver:
    ...
    def _demo_loop(self):
        ...
        import random
        demo_messages = [
            (0x100, [0x12, 0x34, 0x50, 0x3C, 0x78, 0x00, 0x00, 0x00]),
            (0x200, [0x01, 0xF4, 0x32, 0x64, 0x00, 0x00, 0x00, 0x00]),
            (0x300, [0xAA, 0xBB, 0xCC, 0xDD, 0x00, 0x00, 0x00, 0x00]),
        ]
