...
def get_messages():
    ...
    with messages_lock:  # Защищаем доступ к can_messages
        ...
        print(f"📡 API /messages запрос. Всего сообщений: {len(can_messages)}, "
              f"отправляется: {len(filtered_data)}")
...
def generate_single_chart_html(signal_key, use_real_time):
    ...
    <body>
        ...
        <script>
            // Добавьте JavaScript для графика здесь
        </script>
    </body>
    </html>
    """
...
def get_single_chart_data(signal_key, time_window, real_time):
    """Получение данных для одного графика"""
    # Реализуйте получение данных для конкретного сигнала
    # Это зависит от вашей структуры хранения данных
    return {
        "values": [],  # Заполните реальными данными
        "timestamps": [],  # Заполните реальными данными
        "label": signal_key,
        "last_message": {}  # Последнее сообщение для логгера
    }
