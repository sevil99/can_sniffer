...
def process_can_message(can_message, channel):
    ...
    if can_messages and len(can_messages) % 10 == 0:  # Выводим каждое 10-е сообщение
...
def generate_charts_html(signal_keys, use_real_time, include_loggers):
    """Генерация HTML для нескольких графиков"""
    # Здесь реализуйте генерацию HTML с графиками
    ...
    for signal_key in signal_keys:
        ...
        <div class="chart-container">
            <h3>{message_id} - {signal_name}</h3>
            <div class="chart-wrapper">
                <canvas id="chart_{signal_key}"></canvas>
            </div>
        """
        ...
        if include_loggers:
            ...
            <div class="logger-container">
                <h4>Лог сообщений</h4>
                <div class="logger-messages" id="logger_{signal_key}"></div>
            </div>
            """
...
def generate_single_chart_html(signal_key, use_real_time):
    """Генерация HTML для одного графика"""
    parts = signal_key.split('_')
    message_id = parts[0]
    signal_name = '_'.join(parts[1:-1])
    ...
    <html>
    <head>
        <title>CAN Chart: {message_id} - {signal_name}</title>
    ...
    <body>
        <div id="chart-container">
            <h1>{message_id} - {signal_name}</h1>
            <canvas id="mainChart"></canvas>
        </div>
        <script>
            // Здесь будет код для построения графика
            console.log("Chart for {signal_key} would be generated here");
        </script>
    </body>
    </html>
    """

def get_single_chart_data(signal_key, time_window, real_time):
    """Получение данных для одного графика"""
    # Упрощенная реализация - на практике здесь нужно получать реальные данные
    if signal_key in signal_data:
        data = list(signal_data[signal_key])
        if data:
            current_time = time.time()
            recent_data = [
            ...
            if recent_data:
                ...
                return {
                    ...
                    "label": signal_key,
                    "last_message": recent_data[-1] if recent_data else {}
                }
...
def chart_data_single():
    ...
    real_time = request.args.get('real_time', 'true') == 'true'
    current_time = time.time()
...
@app.route('/api/export_chart_html', methods=['POST'])
def export_chart_html():
    """Экспорт одного графика в HTML с Plotly"""
    try:
        data = request.json
        signal_key = data.get('signal_key')
        use_real_time = data.get('use_real_time', True)
        
        if not signal_key:
            return jsonify({"status": "error", "message": "No signal key provided"})
        
        # Получаем данные для графика
        current_time = time.time()
        time_window = 60  # 60 секунд по умолчанию
        ...
        if signal_key in signal_data and signal_data[signal_key]:
            ...
            if recent_data:
                ...
                signal_name = '_'.join(signal_name_parts) if signal_name_parts else "Signal"
                channel = config.get('channel', '1')
                
                # Создаем Plotly график
                if use_real_time:
                    # Конвертируем временные метки в читаемый формат
                    x_values = [datetime.fromtimestamp(ts).strftime('%H:%M:%S.%f')[:-3] for ts in timestamps]
                else:
                    # Относительное время
                    min_time = min(timestamps)
                    x_values = [f"{(ts - min_time):.2f}с" for ts in timestamps]
                ...
                fig.add_trace(go.Scatter(
                    ...
                    y=values,
                    mode='lines+markers',
                    name=f'{message_id} - {signal_name}',
                    line=dict(
                        color='rgb(52, 152, 219)' if channel == '0' else 'rgb(231, 76, 60)',
                        width=2
                    ),
                    marker=dict(size=4),
                    fill='tozeroy',
                    fillcolor='rgba(52, 152, 219, 0.2)' if channel == '0' else 'rgba(231, 76, 60, 0.2)'
                ))
                
                fig.update_layout(
                    title=f'CAN Signal: {message_id} - {signal_name} (Channel {channel})',
                    xaxis_title='Time',
                    yaxis_title='Value',
                    hovermode='x unified',
                    showlegend=True,
                ...
                return Response(
                    html_content,
                    mimetype="text/html",
                    headers={"Content-Disposition": f"attachment;filename=can_chart_{signal_key}.html"}
                )
        
        # Если нет данных, возвращаем пустой шаблон
        empty_html = f'''
        <!DOCTYPE html>
        <html>
        <head>
            <title>CAN Chart: {signal_key}</title>
            <script src="https://cdn.plot.ly/plotly-2.24.1.min.js"></script>
        </head>
        <body>
            <div id="plot"></div>
            <script>
                // No data available for {signal_key}
                document.getElementById('plot').innerHTML = '<h3>No data available for {signal_key}</h3>';
            </script>
        </body>
        </html>
        '''
...
@app.route('/api/export_chart_plotly', methods=['POST'])
def export_chart_plotly():
    """Экспорт графика в Plotly HTML файл"""
    try:
        data = request.json
        signal_key = data.get('signal_key')
        ...
        # Получаем данные из chart_data_single
        chart_data_response = chart_data_single()
        chart_data = json.loads(chart_data_response.get_data(as_text=True))
        
        if not chart_data or not chart_data.get('values'):
            return jsonify({"status": "error", "message": "No data available"})
        
        # Создаем более продвинутый Plotly график
        fig = go.Figure()
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(
                color='rgb(52, 152, 219)' if chart_data.get('channel') == '0' else 'rgb(231, 76, 60)',
                width=3
            ),
            marker=dict(
                size=6,
            ...
            fill='tozeroy',
            fillcolor='rgba(52, 152, 219, 0.2)' if chart_data.get('channel') == '0' else 'rgba(231, 76, 60, 0.2)'
        ))
        
        # Настройки layout
        fig.update_layout(
            title=dict(
                text=f"<b>CAN Monitor - {chart_data['label']}</b>",
        ...
        fig.add_annotation(
            ...
            xref="paper",
            yref="paper",
            text=f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}<br>Data points: {len(chart_data['values'])}",
            showarrow=False,
        ...
        <script>
            ...
            function startAutoUpdate() {
                ...
                updateInterval = setInterval(updateData, 5000); // Обновлять каждые 5 секунд
            }
            ...
            console.log('CAN Plotly Chart loaded. Press SPACE to toggle auto-update.');
        </script>
        '''
        
        # Формируем информацию для панели
        info_html = f'''
        <div class="info-panel">
            <div class="info-item">
                <div class="info-label">CAN ID:</div>
                <div class="info-value">{chart_data.get('id', 'N/A')}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Signal Name:</div>
                <div class="info-value">{chart_data.get('signal', 'N/A')}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Channel:</div>
                <div class="info-value">CH{chart_data.get('channel', '1')}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Data Points:</div>
                <div class="info-value">{len(chart_data['values'])}</div>
            </div>
            <div class="info-item">
                <div class="info-label">First Bytes:</div>
                <div class="info-value">{chart_data.get('first_bytes', 'All')}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Scale:</div>
                <div class="info-value">{chart_data.get('config', {}).get('scale', 1.0)}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Offset:</div>
                <div class="info-value">{chart_data.get('config', {}).get('offset', 0.0)}</div>
            </div>
        </div>
        '''
        
        # Объединяем все части
        html_content = html_content.replace('<head>', f'<head>{extra_css}')
        html_content = html_content.replace('</body>', f'{extra_js}</body>')
        html_content = html_content.replace('<div id="plotly-graph-div"></div>', 
                                           f'<div class="container"><div class="header"><h1>CAN Monitor - Plotly Export</h1></div>{info_html}<div class="chart-container"><div id="plotly-graph-div"></div></div><div class="footer">Generated by CAN Monitor v6 | {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</div></div>')
        
        return Response(
            html_content,
            mimetype="text/html",
            headers={"Content-Disposition": f"attachment;filename=can_plotly_{signal_key}.html"}
        )
        
    except Exception as e:
        print(f"❌ Ошибка экспорта Plotly: {e}")
        return jsonify({"status": "error", "message": str(e)})
...
if __name__ == '__main__':
    print("=" * 60)
    print("🚀 CAN Monitor Web Interface v6.0 - Исправленная версия")
    print("=" * 60)
    print("📊 Функциональность:")
    print("  • Отдельные экземпляры приемников для каждого канала")
    print("  • Исправлена ошибка 'Выберите хотя бы один канал'")
    print("  • Автоматический переход в демо-режим при отсутствии оборудования")
