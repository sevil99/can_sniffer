class CANMonitor {
    constructor() {
        ...
        console.log('CAN Monitor v6 - Оффлайн версия с автосохранением');
        console.log('Функции: автосохранение каждые 10000 сообщений, история до 30 минут');
    }
    ...
    initializeEventListeners() {
        ...
        // Управление историей
        $('#applyHistorySettings').click(() => this.applyHistorySettings());
        $('#showAutoSaveFilesBtn').click(() => this.showAutoSaveFiles());

        // Установка значений по умолчанию
        $(document).ready(() => {
            $('#channelSelect0').prop('checked', false);
            $('#channelSelect1').prop('checked', true);
            $('#historyMinutes').val(this.historyMinutes);
            $('#maxPoints').val(this.maxPoints);
    ...
    async applyHistorySettings() {
        this.historyMinutes = parseInt($('#historyMinutes').val());
        this.maxPoints = parseInt($('#maxPoints').val());
        ...
        try {
            const response = await fetch('/api/settings/history', {
                ...
                body: JSON.stringify({
                    max_history_minutes: this.historyMinutes,
                    max_history_points: this.maxPoints
                })
            });

            const data = await response.json();
            if (data.status === 'success') {
                this.showNotification('✅ Настройки истории применены', 'success');
                this.updateCharts();
            } else {
                this.showNotification('❌ Ошибка применения настроек: ' + data.message, 'error');
            }
        } catch (error) {
            console.error('Ошибка применения настроек:', error);
            this.showNotification('❌ Ошибка применения настроек: ' + error, 'error');
        }
    }
    ...
    async connect() {
        ...
        try {
            ...
            if (data.status === 'connected') {
                ...
                // Сбрасываем счетчики на клиенте
                this.totalMessageCount = 0;
                this.saveCounter = 0;
                this.recordingStartTime = null;
                ...
                this.showNotification(`✅ Успешно подключено к каналу ${channel}! Автосохранение каждые 10000 сообщений`, 'success');
    ...
    async disconnect() {
        try {
            ...
            if (data.status === 'disconnected') {
                ...
                // Скрываем индикатор автосохранения
    ...
    async updateStatus() {
        try {
            ...
            this.saveCounter = data.save_counter || 0;
            this.recordingStartTime = data.recording_start_time ? new Date(data.recording_start_time) : null;

            const indicator = $('#statusIndicator');
            const statusText = $('#statusText');
            const messageCountElement = $('#messageCount');
            const totalMessageCountElement = $('#totalMessageCount');
            const historyStatusElement = $('#historyStatus');
            const saveCounterElement = $('#saveCounter');
            const autoSaveFoldersElement = $('#autoSaveFolders');
            const recordingStartTimeElement = $('#recordingStartTime');
            const recordingDurationElement = $('#recordingDuration');
            const historyMessagesCountElement = $('#historyMessagesCount');
            const messageFrequencyElement = $('#messageFrequency');
            ...
            messageCountElement.text(`Сообщений: ${this.messageCount}`);
            totalMessageCountElement.text(this.totalMessageCount);
            saveCounterElement.text(this.saveCounter);
            autoSaveFoldersElement.text(this.saveCounter);

            if (data.history_message_count > 0) {
                historyMessagesCountElement.text(data.history_message_count);
                let historyText = `${data.history_message_count} сообщений`;
                if (this.recordingStartTime) {
                    historyText += ` (${this.formatDuration((new Date() - this.recordingStartTime) / 1000)})`;
                }
                historyStatusElement.text(historyText);
            } else {
                historyMessagesCountElement.text('0');
                historyStatusElement.text('Нет данных');
            }

            if (this.recordingStartTime) {
                recordingStartTimeElement.text(this.recordingStartTime.toLocaleTimeString());
                const duration = Math.floor((new Date() - this.recordingStartTime) / 1000);
                recordingDurationElement.text(this.formatDuration(duration));

                if (duration > 0) {
                    const frequency = this.totalMessageCount / duration;
                    messageFrequencyElement.text(`${frequency.toFixed(1)}/сек`);
                }
            }
...
<body>
    ...
    async updateCharts() {
        try {
            ...
            // Используем историю вместо реального времени
            const response = await fetch(`/api/chart_data?time_window_minutes=${this.historyMinutes}&max_points=${this.maxPoints}&real_time=${realTimeParam}`);
            const chartData = await response.json();
    ...
    renderStatistics(stats) {
        ...
        const mainStats = [
            ...
            { label: 'История сообщений', value: stats.history_messages || 0 },
    ...
    async exportData(format) {
        try {
            if (format === 'csv') {
                // Используем новый endpoint для CSV
                ...
                if (response.ok) {
                    const blob = await response.blob();
                    this.downloadFile(blob, `can_messages_${Date.now()}.csv`, 'text/csv');
                    this.showNotification('✅ CSV файл успешно экспортирован', 'success');
                } else {
                    const error = await response.json();
                    throw new Error(error.message || 'Ошибка экспорта CSV');
                }
            } else if (format === 'json') {
    ...
    async showAutoSaveFiles() {
        this.showNotification('📂 Проверьте папку с проектом на наличие папок автосохранения', 'info');
    }
...
$(document).ready(() => {
    ...
    console.log('CAN Monitor v6 с автосохранением и историей инициализирован');
    console.log('Функции: автосохранение каждые 10000 сообщений, история до 30 минут');
});