class CANMonitor {
    ...
    async connect() {
        ...
        // ВАЖНОЕ ИСПРАВЛЕНИЕ: Используем первый выбранный канал
        const channel = this.selectedChannels[0];
        console.log('Подключение к каналу:', channel);

        try {
            console.log('Подключение к CAN сети...');
            ...
            const response = await fetch('/api/connect', {
                ...
                body: JSON.stringify({
                    channel: channel,  // ВАЖНО: 'channel' (единственное число), а не 'channels'
                    baud_rate: parseInt(baudRate)
                })
            });
    ...
    renderSelectedSignalsList() {
        ...
        this.selectedSignals.forEach((config, signalKey) => {
            ...
            // ИСПРАВЛЕНИЕ: Правильное извлечение имени сигнала
            let signalName = '';
    ...
    renderSingleChart(signalKey, data, config) {
        ...
        // ИСПРАВЛЕНИЕ: Проверяем, что data существует
